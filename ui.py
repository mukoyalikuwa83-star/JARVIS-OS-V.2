...
def _assistant_name() -> str:
    """Resolve the configurable assistant display name."""
    try:
...
# centralized makes the Settings UI, the renderer, and assistant voice commands
...
class ChatBubbleWidget(QWidget):
    ...
    def refresh_theme(self):
        ...
        prefixes = {"user": "You: ", "ai": "Assistant: ", "file": "FILE: ", "error": "ERR: ", "sys": "SYS: "}
    ...
    def _typing_tick(self):
        ...
        if self._typing_idx > len(self._typing_body):
            ...
            return
        partial = 'Assistant: ' + self._typing_body[:self._typing_idx]
        if hasattr(self, '_typing_bubble') and self._typing_bubble:
            self._c_lay.removeWidget(self._typing_bubble)
            self._typing_bubble.deleteLater()
        lbl = QLabel(partial + '▌')
    ...
    def _on_message(self, text: str):
        ...
        if text.lower().startswith(("jarvis:", "assistant:")) and not getattr(self, '_skip_typing', False):
            prefix = "assistant:" if text.lower().startswith("assistant:") else "jarvis:"
        ...
        if tl.startswith("you:"):
            ...
            text_col = C.WHITE
            name = "YOU"
        elif tl.startswith(("jarvis:", "assistant:")):
            sender = "ai"
            prefix = "assistant:" if tl.startswith("assistant:") else "jarvis:"
...
class FocusDialogueWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._speaker_lbl = QLabel(_assistant_name())
        ...
        self._input = QLineEdit()
        self._input.setPlaceholderText("Give the assistant a command")
    ...
    def _apply_message(self, text: str):
        ...
        elif lower.startswith(("jarvis:", "assistant:")):
            prefix = "assistant:" if lower.startswith("assistant:") else "jarvis:"
            speaker, body, color = _assistant_name(), clean[len(prefix):].strip(), C.PRI
...
class ResearchProgressWidget(QWidget):
    """Compact, factual progress surface for a long-running task."""
...
class AgentGridWidget(QWidget):
    """
    Vertical timeline agent display – polished edition.
...
class TaskQueueWidget(QWidget):
    """Displays a live task queue parsed from assistant log messages."""
...
class LogWidget(QTextEdit):
    ...
    def _next(self):
        ...
        elif tl.startswith(("jarvis:", "assistant:")): self._tag = "ai"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure the assistant before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class _OverlayBase(QWidget):
    """
    Base class for all overlay popups.
    Provides:
...
class SettingsOverlay(_OverlayBase):
    ...
    def __init__(self, parent=None, current_name: str = "",
        ...
        self._s_name_input.setPlaceholderText("e.g. Alex")
...
class NameSignInOverlay(_OverlayBase):
    """Overlay that asks the user for their name so the assistant can address them personally."""
    ...
    def __init__(self, parent=None, existing_name: str = ""):
        ...
        sub_txt   = f"Currently: {existing_name}" if existing_name else "The assistant needs to know who it's talking to."
...
class VoiceSelectOverlay(_OverlayBase):
    """Popup overlay for selecting the assistant voice (used in first-run setup flow)."""
    ...
    def __init__(self, parent=None, current_voice: str = "puck"):
        ...
        layout.addWidget(_lbl("Choose the voice the assistant will speak with.", 9, color=C.PRI_DIM))
        layout.addSpacing(4)
...
class _SubtitleWidget(QWidget):
    ...
    def set_text(self, text: str):
        ...
        # which is called externally only when the assistant finishes talking (turn_complete).
    ...
    def start_hold_timer(self):
        """Start (or restart) the fade-out hold timer. Call this when the assistant finishes speaking."""
        if self._chunks:
            self._hold_timer.stop()
...
class VisionPreviewWindow(QWidget):
    """Small draggable live preview shown while the assistant is using vision."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setObjectName("visionPreview")
        self.setAccessibleName("Assistant live vision preview")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        super().__init__()
        _load_bundled_fonts()
        self.setWindowTitle(_assistant_name())
        ...
        try:
            if hasattr(self, "_tray") and self._tray.isVisible() and not getattr(self, "_force_quit", False):
                ...
                self._tray.showMessage(
                    _assistant_name(), "Running in background. Click tray icon to restore.",
    ...
    def _setup_system_tray(self):
        ...
        self._tray.setToolTip(_assistant_name())
        ...
        show_action = QAction(f"Show {_assistant_name()}", self)
        ...
        quit_action = QAction(f"Quit {_assistant_name()}", self)
    ...
    def _request_quit(self):
        """Use one clean shutdown path for tray, UI, and assistant self-quit."""
    ...
    def _handle_ui_command(self, action: str):
        ...
        if action in {"quit jarvis", "quit_jarvis", "quit", "quit assistant", "exit", "quit_ui"}:
            self._request_quit()
            return True
    ...
    def _announce_command_center_modules(self):
        ...
        if self.on_text_command:
            ...
            self._log.append_log(f"Assistant: {announcement}")
    ...
    def _build_header(self) -> QWidget:
        ...
        sub_stark = QLabel("LOCAL AI ASSISTANT")
    ...
    def _build_footer(self) -> QWidget:
        ...
        w.setAccessibleName("Assistant command rail")
        w.setFixedHeight(72)
        lay = QHBoxLayout(w)
        ...
        self._rail_status_dot.setAccessibleName("Assistant status indicator")
        ...
        self._rail_mode_lbl.setAccessibleName("Assistant current state")
        ...
        self._tts_btn.setToolTip("Change voice")
        self._tts_btn.setAccessibleName("Change voice")
        ...
        self._quit_btn.setAccessibleName(f"Quit {_assistant_name()}")
        self._quit_btn.setToolTip(f"Quit {_assistant_name()}")
    ...
    def _send(self, txt: str = ""):
        ...
        try:
            ...
            if normalized in {
                ...
                self._set_command_center(True)
                self._log.append_log("Assistant: Command Center is open.")
                return
            if normalized in {
                ...
                self._set_command_center(False)
                self._log.append_log("Assistant: Returning to focus view.")
                return
            if self.on_text_command:
                def _dispatch():
                    try:
            ...
            # Keep the assistant alive and surface the problem in its own log instead.
            try:
                self._log.append_log(f"ERR: Message processing failed: {exc}")
            except Exception:
                print(f"[Assistant] Message processing failed: {exc}")
    ...
    def _apply_state(self, state: str):
        ...
        if state == "THINKING":
            self._show_toast("Thinking...", "info")
    ...
    def _on_name_done(self, name: str):
        ...
        try:
            ...
            self._log.append_log(f"Assistant: At your service, {save_name}.")
            self._log.append_log("Assistant: All systems nominal. How may I assist you today?")
        except Exception as e:
...
class JarvisUI:
    ...
    def set_mic_state(self, state: str):
        """Update mic activity indicator: 'listening', 'detecting', 'speaking', 'barge_in'."""
        self._win._mic_state = state
    ...
    def start_subtitle_hold(self):
        """Thread-safe: start the subtitle fade-out hold timer (call when the assistant finishes talking)."""
        try:
    ...
    def set_theme(self, theme_key: str):
        """Thread-safe theme change requested by the assistant."""
        self._win._theme_sig.emit(theme_key)

    def set_graphics_quality(self, quality: str):
        """Thread-safe graphics quality change requested by the assistant."""
