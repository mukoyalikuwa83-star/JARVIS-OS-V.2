# Assistant — Download, Install, and First Run
...
- **[The 5-Minute Path](#the-5-minute-path-no-terminal-needed)** — download
  the ZIP, double-click a launcher, talk to the assistant. Works on macOS and
...
1. **A Google Gemini API key.** Get one at
   ...
   Gemini Live models the assistant uses do require a key that has Live API
...
2. **A working microphone** (built-in is fine) and permission for
   the assistant to use it.
3. **A desktop operating system the assistant supports:** macOS (Intel or
...
4. Move the unzipped folder somewhere stable (e.g. `~/Applications/`
   ...
   OneDrive, or Downloads — the assistant keeps a virtual environment in the
...
1. In the project folder, find the file `.env.example` and **make a copy
...
1. In Finder, open the `scripts/` folder inside the project folder.
...
4. Subsequent launches start the assistant in seconds.
5. macOS will ask for microphone permission the first time the assistant
   tries to use the mic — click **Allow**.

**Windows:**

1. In File Explorer, open the `scripts/` folder inside the project folder.
...
When the assistant is ready you'll see its UI and a small log panel. Say:
...
```
[Assistant] 🔑 Validating Gemini API key...
[Assistant] ✅ Gemini API key validated.
[Awareness] 👁️ Awareness engine started
[Assistant] 🔌 Connecting...
[Assistant] ✅ Connected.
[Assistant] 🎤 Mic started
```
...
When the assistant says its greeting out loud, the first run is complete.
...
The assistant also accepts typed input in the UI's text field — useful when
...
2. Confirm it's in the project root, **the same folder that
   contains `main.py`**.
...
The assistant uses its own controlled browser profile for Instagram DMs. The
first time you ask it to message someone on Instagram, it may open a
separate browser window and ask you to log in there.

1. Log into Instagram in the browser window.
2. Leave that window signed in.
3. Ask to send the Instagram message again.

The assistant will type the draft and ask before sending. It should not require
...
- **macOS:** System Settings → Privacy & Security → Microphone. Make
  sure Terminal (or whichever app launched the assistant) is checked. Quit
...
The assistant will print a warning and auto-select a fallback Live model from
...
whatever launched the assistant). macOS asks per-app; rerun the send
after granting.

### Reset everything

If the venv is corrupted or you just want a clean slate, **delete the
`.venv` folder** inside the project directory. The next launcher run
...
- **ZIP users:** download a fresh ZIP and replace your project folder.
