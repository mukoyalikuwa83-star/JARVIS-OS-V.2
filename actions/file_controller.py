...
def _default_safe_roots() -> list[Path]:
    """Default access scope: every mounted drive/volume, not just the home dir.

    The user can narrow this with the JARVIS_ALLOWED_ROOTS environment
    variable (colon/;-separated paths).
    """
    roots: list[Path] = []
    try:
        if _OS == "Windows":
...
def _configured_safe_roots() -> list[Path]:
    configured = os.environ.get("JARVIS_ALLOWED_ROOTS", "").strip()
    if not configured:
        return _default_safe_roots()
    return [Path(p.strip()).expanduser() for p in configured.replace(";", ":").split(":") if p.strip()]


_SAFE_ROOTS: list[Path] = _configured_safe_roots()
...
def create_file(path: str, name: str = "", content: str = "") -> str:
    try:
        ...
        write_text_with_stamp(
            target,
            content,
            f"Created through the file_controller create_file action for {target.name}.",
        )
        ...
        if src.is_dir():
            ...
            for child in dst.rglob("*"):
                if child.is_file() and not child.name.endswith(".jarvis_meta.json"):
                    write_sidecar_metadata(child, f"Copied from {src}.")
        else:
            shutil.copy2(str(src), str(dst))
            write_sidecar_metadata(dst, f"Copied from {src}.")
...
def read_file(path: str, name: str = "", max_chars: int = 50000) -> str:
    try:
        base   = _resolve_path(path)
        target = (base / name) if name else base
        if not _is_safe_path(target):
    ...
    try:
        ...
        if not append or not existed_before:
            write_text_with_stamp(
                target,
                content,
                f"Written through the file_controller write action for {target.name}.",
            )
...
def write_file_direct(full_path: str, content: str) -> str:
    """Write content directly to an absolute file path."""
    try:
        ...
        write_text_with_stamp(
            target,
            content,
            f"Written through file_controller for {target.name}.",
        )
        return f"Written to: {target} ({len(content)} chars)"
    except Exception as e:
        return f"Could not write file: {e}"


def run_file(path: str, name: str = "", args: list = None) -> str:
    """Run a file with the OS default interpreter."""
    try:
        base   = _resolve_path(path)
        target = (base / name) if name else base
        if not target.exists():
        ...
        if target.is_dir():
            return f"Cannot run a directory: {target}"
        ...
        interp = interpreters.get(target.suffix.lower())
        if not interp:
            os.startfile(str(target))
            return f"Opened {target.name} with default app"

        result = subprocess.run(
            interp + [str(target)] + (args or []),
            capture_output=True, text=True,
            encoding="utf-8", errors="replace",
            timeout=30, cwd=str(target.parent),
        )
        parts = []
        ...
        return "\n\n".join(parts) if parts else f"Ran {target.name} with no output"
    except subprocess.TimeoutExpired:
        return f"Ran {target.name} — timed out after 30s (may still be running)"
    except Exception as e:
        return f"Run error: {e}"


def find_files(name: str = "", extension: str = "",
               path: str = "home", max_results: int = 200) -> str:
    try:
        ...
        results    = []
        dir_count  = 0
        max_dirs   = 5000  # performance guard only, not an access restriction
...
def get_largest_files(path: str = "downloads", count: int = 10) -> str:
    count = min(count, 500)  # generous cap, adjustable via count
    try:
        search_path = _resolve_path(path)
        if not _is_safe_path(search_path):
            return f"Access denied: {search_path}"
...
def file_controller(
    ...
    path   = params.get("path", params.get("file_path", "desktop"))
    ...
    try:
        if action == "list":
            ...
            full_path = params.get("file_path", "")
            if full_path and not name:
                content = params.get("content", "")
                return write_file_direct(full_path, content)
            return write_file(
                path, name=name,
                content=params.get("content", ""),
            ...
            return run_file(path, name=name, args=params.get("args", []))
            ...
            return find_files(
                ...
                path=path,
                max_results=min(int(params.get("max_results", 200)), 500),
            )
