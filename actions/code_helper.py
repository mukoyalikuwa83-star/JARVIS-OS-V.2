...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"code_output{ext}"
...
def _save_file(path: Path, content: str, context: str = "Generated by the code_helper.") -> str:
    try:
...
def _take_screenshot() -> Path | None:
    try:
        ...
        mark_created_file(
            screenshot_path,
            "Screenshot captured by the code_helper for screen debugging.",
        )
...
Updated code:"""
    ...
    status = _save_file(Path(file_path), edited, f"Edited existing code file: {instruction}")
    ...
    if action == "write":
        if code and file_path:
            try:
                ...
                write_text_with_stamp(p, code, f"Written by code_helper for: {description or file_path}")
                return f"Code written to: {file_path} ({len(code)} chars)"
            except Exception as e:
