...
from core.identity import assistant_name
...
def _is_jarvis_self_shutdown_request(params: dict, action: str, description: str, value) -> bool:
    ...
    if assistant_name().lower() not in text:
        return False
...
- If the user says to shut down, restart, reboot, close, quit, exit, or stop the assistant, do not choose shutdown or restart.
...
def computer_settings(
    ...
    if _is_jarvis_self_shutdown_request(params, action, description, value):
        return "The assistant's self-shutdown is disabled. No computer shutdown was performed."
