...
UNSAFE_TYPING_APPS = set(
    app.strip()
    for app in os.environ.get("JARVIS_BLOCKED_TYPING_APPS", "").split(",")
    if app.strip()
)
...
class MouseClickBlocker:
    """Temporarily swallows user mouse clicks while the assistant is typing."""
...
def validate_safe_text_target(purpose: str = "generic", allow_pointer_fallback: bool = False) -> SafeTextEntryResult:
    ...
    if _actual_os() == "windows":
        try:
            import pyautogui
            front = pyautogui.getActiveWindow()
            title = str(getattr(front, "title", "") or "") if front else ""
            if purpose_key == "message" and any(k in title.lower() for k in ("whatsapp", "telegram", "discord", "messages", "signal", "slack", "instagram")):
                return SafeTextEntryResult(True, f"Safe messaging app focused: {title}", title, safety_path="pyautogui")
            return SafeTextEntryResult(True, f"Safe text entry on Windows: {title}", title, safety_path="pyautogui")
        except Exception:
            return SafeTextEntryResult(True, "Safe text entry on Windows (pyautogui fallback).", "unknown", safety_path="pyautogui")
    if _actual_os() != "mac":
        return SafeTextEntryResult(True, "Safe text entry (non-macOS fallback).", "unknown", safety_path="fallback")
