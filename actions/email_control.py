...
are typed into the assistant's controlled browser and sent only after a later explicit
...
def _load_gmail_credentials():
    ...
    if not payload:
        raise GmailConnectionError(
            ...
            "then tell the assistant: connect Gmail using this file."
        )
    try:
...
def _gmail_connect(credentials_path: str = "") -> str:
    ...
    if path is None:
        return (
            ...
            "download the JSON, then drag it into the assistant window and say: connect Gmail using this file."
        )
    try:
        ...
        credentials = flow.run_local_server(
            ...
            access_type="offline",
            prompt="consent",
            success_message="Gmail is connected. You can close this browser tab.",
        )
...
async def _assist_gmail_sign_in(page, player=None, timeout_seconds: float = 90.0) -> bool:
    ...
    The assistant never reads or stores a Google password. The persistent browser profile
...
async def _open_gmail_compose_fields(session, player=None):
    ...
    if signed_out:
        if not await _assist_gmail_sign_in(page, player=player):
            return page, None, (
                ...
                "Chrome window, then ask me to prepare the email again. The assistant never stores your password."
            )
        try:
            await page.goto(compose_url, wait_until="domcontentloaded", timeout=45_000)
        except Exception:
    ...
    if subject is None or body is None:
        ...
        return page, None, "Gmail opened, but I could not find a new compose window. Open Gmail once, finish any sign-in prompts, and try again."
    ...
    if to_field is None:
        return page, None, "Gmail opened a compose window, but I could not locate the recipient field."
...
async def _gmail_web_prepare_once(session, draft: dict[str, str], player=None) -> str:
    ...
    for field_name, selectors in (
        ...
        if locator is None:
            return f"Gmail opened the draft, but I could not locate the {field_name} field. Nothing was sent."
...
async def _gmail_web_prepare_async(session, draft: dict[str, str], player=None) -> str:
    ...
    return (
        "Gmail interrupted the compose operation twice, so I discarded the incomplete draft. "
...
async def _gmail_web_send_async(session, draft: dict[str, str]) -> str:
    ...
    if "mail.google.com" not in str(page.url or "").lower():
        return "The Gmail draft is no longer the active browser tab. Nothing was sent."
    ...
    if send_button is None:
        return "I could not find Gmail's Send button. Nothing was sent."
...
def _mail_error(result: subprocess.CompletedProcess, operation: str) -> str:
    ...
    if "not authorized" in detail.lower() or "not permitted" in detail.lower():
        return (
            ...
            "Allow the assistant or Terminal to control Mail in System Settings > Privacy & Security > Automation."
        )
...
def _prepare_email(params: dict, player=None) -> str:
    ...
    return (
        ...
        + ("I opened Gmail and visibly typed this draft. It remains open for your review.\n" if provider == "gmail" else "")
...
def email_control(parameters: dict | None = None, response=None, player=None, session_memory=None) -> str:
    ...
    if action == "disconnect":
        _clear_gmail_credentials()
        return (
            "The local Gmail connection was removed from the assistant. To revoke Google-side access too, "
            "remove the assistant from your Google Account connections."
        )
    if action == "inbox":
    ...
    if action == "prepare":
        result = _prepare_email(params, player=player)
        if not str(result).startswith("EMAIL_APPROVAL_REQUIRED|"):
            return result
        from core.autonomy import is_autonomous
        if not is_autonomous():
            return result
        # Autonomous mode: no confirmation gate — send the just-prepared email.
        ...
        if draft.get("provider") == "apple_mail" and _SYSTEM == "Darwin":
