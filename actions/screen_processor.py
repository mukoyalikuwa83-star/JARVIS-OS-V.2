...
def _save_config_key(key: str, value) -> None:
    try:
        ...
        _safe_print(f"[Vision] ⚠️  Could not save config key '{key}': {e}")
...
def _compress(img_bytes: bytes, source_format: str = "PNG") -> tuple[bytes, str]:
    ...
    try:
        ...
        _safe_print(f"[Vision] ⚠️  Image compress failed: {e}")
        return img_bytes, f"image/{source_format.lower()}"
...
def _detect_camera_index() -> int:
    ...
    for idx in range(6):
        if _probe_camera(idx, backend):
            ...
            return idx
        _safe_print(f"[Vision] ⚠️  Camera index {idx}: no usable frame")
...
class _VisionSession:
    ...
    def request(self, image_bytes: bytes, mime_type: str, user_text: str, timeout: float = 8.0) -> Optional[str]:
        ...
        try:
            ...
            _safe_print(f"[Vision] ⚠️  Error waiting for vision result: {e}")
            return None
        finally:
            with self._result_lock:
        ...
        while True:
            ...
            try:
                ...
                for exc in eg.exceptions:
                    _safe_print(f"[Vision] ⚠️  Session error: {exc}")
            finally:
                self._session = None
                self._ready_evt.clear()

            _safe_print(f"[Vision] 🔄 Reconnecting in {backoff:.0f}s...")
    ...
    async def _send_loop(self) -> None:
        while True:
            ...
            try:
                ...
                _safe_print(f"[Vision] 📤 Sent {len(image_bytes):,} bytes — '{user_text[:60]}'")
            except Exception as e:
                _safe_print(f"[Vision] ⚠️  Send error: {e}")
    ...
    async def _recv_loop(self) -> None:
        ...
        while True:
            ...
            try:
                async for response in self._session.receive():
                    ...
                    if sc.turn_complete:
                        ...
                        if result_text and self._player:
                            self._player.write_log(f"Assistant: {result_text}")
                            _safe_print(f"[Vision] 💬 {result_text}")
                ...
                _safe_print(f"[Vision] ⚠️  Recv error: {e}")
                raise
    ...
    async def _play_loop(self) -> None:
        ...
        try:
            ...
            _safe_print(f"[Vision] ❌ Play error: {e}")
            raise
        finally:
            stream.stop()
            stream.close()
...
def screen_process(
    ...
    def _deliver(message: str, *, error: bool = False) -> str:
        ...
        if callable(speak):
            try:
                ...
                _safe_print(f"[Vision] ⚠️  Speech handoff failed: {exc}")
        if player and not spoken:
            try:
                prefix = "ERR" if error else "Assistant"
    ...
    _safe_print(f"[Vision] angle={angle!r} question='{user_text[:80]}'")

    # Cooldown to prevent excessive captures
    now = time.time()
    if now - _last_capture_time < _MIN_CAPTURE_INTERVAL:
        _safe_print(f"[Vision] ⚠️  Capture too frequent — skipping")
        return _deliver("Screen capture skipped because requests are too frequent.", error=True)

    try:
        if angle == "camera":
            image_bytes, mime_type = _capture_camera()
            _safe_print(f"[Vision] 📷 Camera: {len(image_bytes):,} bytes")
        else:
            image_bytes, mime_type = _capture_screen()
            _safe_print(f"[Vision] Screen: {len(image_bytes):,} bytes")
        _last_capture_time = now
    except Exception as e:
        _safe_print(f"[Vision] Capture error: {e}")
    ...
    if player and hasattr(player, "show_vision_preview"):
        try:
            ...
            _safe_print(f"[Vision] ⚠️  Live preview unavailable: {exc}")

    try:
        result = _direct_vision_answer(image_bytes, mime_type, user_text)
    except Exception as e:
        _safe_print(f"[Vision] ❌ Analysis error: {e}")
        return _deliver(f"Screen analysis failed: {e}", error=True)

    _safe_print(f"[Vision] 💬 {result}")
...
def warmup_session(player=None) -> None:
    try:
        ...
        _safe_print(f"[Vision] ⚠️  Warmup failed: {e}")
