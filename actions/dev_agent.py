...
def _get_api_key() -> str:
    ...
    key = get_gemini_key()
    if not key:
        raise ValueError("GEMINI_API_KEY is not set. Add it to .env or config/api_keys.json.")
    return key
...
Code for {file_path}:"""
    ...
    try:
        ...
        write_text_with_stamp(
            full_path,
            code,
            f"Generated by the dev_agent for project: {project_description}. File purpose: {file_desc}",
        )
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            write_text_with_stamp(
                full_path,
                fixed,
                f"Auto-fixed by the dev_agent after build error for project: {project_description}",
            )
...
def _build_project(
    ...
    for file_info in sorted_files:
        ...
        for attempt in range(4):
            try:
                ...
                if is_503 and attempt < 3:
                    log(f"Model busy (503) — waiting {wait}s before retry...")
                    time.sleep(wait)
                elif attempt < 2:
                    ...
                    log(f"Switching to fallback model: {fallback}")
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if error_type == "dependency_error" and auto_installs < 10:
