...
JARVIS_TEXT_FOOTER = os.environ.get("MESSAGE_FOOTER", "").strip()
...
def normalize_outgoing_message_text(text: str) -> str:
    ...
    footer = JARVIS_TEXT_FOOTER.strip()
    if not re.search(r"[.!?…]$", message) and not (footer and message.endswith(footer)):
        message += "."
    return message
...
def _send_imessage(receiver: str, message: str) -> str:
    if _get_os() != "mac":
        print("[SendMessage] ⚠️ iMessage not available on Windows — falling back to WhatsApp.")
...
def _send_open_browser_chat(platform_name: str, message: str, url_markers: tuple[str, ...]) -> str:
    if _get_os() == "windows":
        return _windows_pyautogui_type_and_enter(message) or f"Could not send in {platform_name} chat."
    ...
    if focus_error:
        if platform_name.lower() == "instagram":
            return focus_error
        if "cannot find the file" in focus_error.lower() or "osascript" in focus_error.lower():
            windows_result = _windows_pyautogui_type_and_enter(message)
            if windows_result:
...
def _windows_pyautogui_type_and_enter(message: str) -> str:
    """Windows fallback: use pyperclip + Ctrl+V to paste, then Enter."""
    try:
        import pyperclip
        import pyautogui
        import time
        ...
        return f"Message sent via clipboard paste (Windows)."
    except ImportError:
        pass
    except Exception as e:
        print(f"[SendMessage] ⚠️ Windows pyperclip fallback failed: {e}")
    try:
        import pyautogui
        import time
        time.sleep(0.3)
        ...
        time.sleep(0.1)
        pyautogui.press("enter")
        return f"Message typed and sent via pyautogui (Windows)."
    except Exception as e:
        print(f"[SendMessage] ⚠️ Windows pyautogui fallback failed: {e}")
        return ""
...
def _fill_open_browser_chat(platform_name: str, message: str, url_markers: tuple[str, ...]) -> str:
    if _get_os() == "windows":
        try:
            ...
            pyperclip.copy(message)
            pyautogui.hotkey("ctrl", "v")
            return "Message filled via clipboard paste (Windows)."
        except Exception:
            pass
        return f"Could not type in {platform_name} chat."
...
def _send_open_browser_draft(platform_name: str, url_markers: tuple[str, ...]) -> str:
    if _get_os() == "windows":
        try:
            ...
            time.sleep(0.1)
            pyautogui.press("enter")
            return "Draft sent via Enter key (Windows)."
        except Exception:
            pass
        return f"Could not send {platform_name} draft."
...
def _screen_find(description: str, retries: int = 2, delay: float = 0.6) -> tuple[int, int] | None:
    ...
    for attempt in range(max(1, retries)):
        try:
            ...
            prompt = (
                f"You are controlling a {w}x{h} screen for the assistant messaging. "
...
def prepare_message_reply(
    ...
    if action in {"prepare", "draft"} and is_autonomous():
        action = "approve"
