...
def _process_image(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("describe", "ocr", "analyze", "read", "extract_text"):
        try:
            ...
            if len(result) > 500 and params.get("save", True):
                ...
                _save_text_output(out, result, f"AI image {action} result generated by the assistant from {path.name}.")
    ...
    if action == "resize":
        ...
        try:
            ...
            _mark_created(out, f"Image resized by the assistant from {path.name}.")
    ...
    if action == "convert":
        ...
        try:
            ...
            _mark_created(out, f"Image converted by the assistant from {path.name} to {fmt.upper()}.")
    ...
    if action == "compress":
        ...
        try:
            ...
            _mark_created(out, f"Image compressed by the assistant from {path.name} at quality {quality}.")
...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("summarize", "extract_text", "translate_hint", "analyze", "reformat"):
        ...
        if action == "extract_text":
            ...
            _save_text_output(out, text, f"Text extracted by the assistant from PDF {path.name}.")
        ...
        try:
            ...
            if len(result) > 600 and params.get("save", True):
                ...
                _save_text_output(out, result, f"PDF {action} generated by the assistant from {path.name}.")
    ...
    if action == "to_word":
        ...
        try:
            ...
            doc.save(out)
            _mark_created(out, f"Word document converted by the assistant from PDF {path.name}.")
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    if action == "extract_text":
        if file_type != "txt":
            ...
            _save_text_output(out, content, f"Text extracted by the assistant from {path.name}.")
    ...
    try:
        ...
        if len(result) > 600 and params.get("save", True):
            out = _output_path(path, action, ".txt")
            _save_text_output(out, result, f"Text document {action} generated by the assistant from {path.name}.")
            return f"{result[:400]}...\n\nFull result saved: {out.name}"
        return result
    except Exception as e:
...
def _process_data(path: Path, file_type: str, action: str,
    ...
    if action in ("convert", "to_csv", "to_excel", "to_json"):
        ...
        try:
            if fmt == "csv":
                ...
                _mark_created(out, f"CSV converted by the assistant from {path.name}.")
            elif fmt == "xlsx":
                out = _output_path(path, "converted", ".xlsx")
                df.to_excel(out, index=False)
                _mark_created(out, f"Excel workbook converted by the assistant from {path.name}.")
            elif fmt == "json":
                out = _output_path(path, "converted", ".json")
                df.to_json(out, orient="records", force_ascii=False, indent=2)
                _mark_created(out, f"JSON data converted by the assistant from {path.name}.")
            return f"Converted to {fmt.upper()}. Saved: {out.name}"
        except Exception as e:
            return f"Convert failed: {e}"
    ...
    if action == "filter":
        ...
        try:
            ...
            _mark_created(out, f"Filtered CSV generated by the assistant from {path.name}.")
    ...
    if action == "sort":
        ...
        try:
            ...
            _mark_created(out, f"Sorted data file generated by the assistant from {path.name}.")
...
def _process_json(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "format":
        ...
        _mark_created(out, f"Formatted JSON generated by the assistant from {path.name}.")
    ...
    if action == "to_csv":
        try:
            ...
            if isinstance(data, list):
                ...
                _mark_created(out, f"CSV converted by the assistant from JSON file {path.name}.")
...
def _process_code(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "run":
        run_commands = {
            ...
            "bat":  ["cmd", "/c", str(path)],
            "cmd":  ["cmd", "/c", str(path)],
            "ps1":  ["powershell", "-ExecutionPolicy", "Bypass", "-File", str(path)],
            "sh":   ["bash", str(path)],
        }
    ...
    try:
        ...
        if action in ("fix", "optimize", "document") and params.get("save", True):
            ...
            _save_text_output(out, code_to_save, f"Code {action} generated by the assistant from {path.name}.")
            return f"{result[:400]}...\n\nSaved: {out.name}"
        return result
    except Exception as e:
        return f"AI processing failed: {e}"
...
def _process_audio(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "transcribe":
        try:
            ...
            if params.get("save", True):
                ...
                _save_text_output(out, result, f"Audio transcript generated by the assistant from {path.name}.")
    ...
    if action == "convert":
        ...
        try:
            ...
            _mark_created(out, f"Audio converted by the assistant from {path.name} to {fmt.upper()}.")
            return f"Converted to {fmt.upper()}. Saved: {out.name}"
        except ImportError:
    ...
    if action == "trim":
        ...
        try:
            ...
            _mark_created(out, f"Audio trimmed by the assistant from {path.name}.")
...
def _process_video(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "extract_audio":
        ...
        try:
            ...
            _mark_created(out, f"Audio extracted by the assistant from video {path.name}.")
    ...
    if action == "trim":
        ...
        try:
            ...
            _mark_created(out, f"Video trimmed by the assistant from {path.name}.")
    ...
    if action == "extract_frame":
        ...
        try:
            ...
            _mark_created(out, f"Video frame extracted by the assistant from {path.name} at {timestamp}.")
    ...
    if action == "compress":
        ...
        try:
            ...
            _mark_created(out, f"Video compressed by the assistant from {path.name} with CRF {crf}.")
            before = _file_size_str(path)
            after  = _file_size_str(out)
            return f"Compressed: {before} → {after}. Saved: {out.name}"
        except Exception as e:
    ...
    if action == "convert":
        ...
        try:
            ...
            _mark_created(out, f"Video converted by the assistant from {path.name} to {fmt.upper()}.")
            return f"Converted to {fmt.upper()}. Saved: {out.name}"
        except Exception as e:
            return f"Convert failed: {e}"
        ...
        try:
            ...
            for child in dest.rglob("*"):
                if child.is_file() and not child.name.endswith(".jarvis_meta.json"):
                    _mark_created(child, f"Extracted by the assistant from archive {path.name}.")
...
def _process_pptx(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("summarize", "extract_text", "analyze"):
        ...
        if action == "extract_text":
            out = _output_path(path, "text", ".txt")
            _save_text_output(out, text, f"Presentation text extracted by the assistant from {path.name}.")
            return f"Text extracted. Saved: {out.name}"
        try:
