"""Cancellable, source-grounded deep research for the assistant."""
...
BASE_DIR = _base_dir()
SYNTHESIS_MODELS = ("gemini-3.5-flash-lite", "gemini-3.6-flash")

DEPTH_CONFIG = {
    "quick": {"queries": 5, "max_sources": 30},
    "standard": {"queries": 12, "max_sources": 60},
    "deep": {"queries": 25, "max_sources": 120},
}
...
class ResearchRequest:
    ...
    def from_parameters(cls, parameters: dict | None) -> "ResearchRequest":
        ...
        focus_areas = [str(item).strip() for item in raw_focus if str(item).strip()][:25]
        ...
        max_sources = max(10, min(200, max_sources))
        return cls(
            question=question,
            depth=depth,
...
def _duckduckgo_html_search(query: str, max_results: int) -> list[dict]:
    ...
    response = requests.get(
        ...
        params={"q": query},
        headers={"User-Agent": "Mozilla/5.0 Research Reports"},
        timeout=18,
    )
...
def _open_visible_research(
    ...
    *,
    ...
    """Drive the assistant's controlled browser through the query and one new source tab."""
...
The assistant reviewed {len(sources)} distinct web sources for this question. The synthesis model was unavailable, so this report preserves the retrieved evidence directly instead of inventing a conclusion. The strongest probable answer should be drawn from the recurring claims below and checked against the linked sources.
...
def _save_report(result: ResearchResult, destination: str, output_path: str = "") -> Path:
    ...
    if output_path:
        ...
        report_path = Path.home() / "Desktop" / "Research Reports" / f"{_slug(result.question)}-{stamp}.md"
    else:
        report_path = Path.home() / "Documents" / "Research Reports" / f"{_slug(result.question)}-{stamp}.md"
