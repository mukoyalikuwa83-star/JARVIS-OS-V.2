"""Create polished, editable PowerPoint presentations for users."""
...
PRESENTATION_QUOTA_COOLDOWN_SECONDS = 30
...
# The application bundles Space Grotesk and JetBrains Mono for Qt, but
...
def _safe_filename(value: str) -> str:
    ...
    cleaned = cleaned[:80] or "Presentation"
...
def _output_path(raw_path: str, title: str) -> Path:
    ...
    if not raw:
        return Path.home() / "Desktop" / "Presentations" / _safe_filename(title)
...
"""
    ...
    prompt = f"""You are an elite presentation editor and data storyteller.
Task mode: {mode}
...
"""
    ...
    if bundle:
        for media_path in bundle.media_paths[:20]:
...
def _render_presentation(
    ...
    prs.core_properties.subject = "Editable presentation created by the assistant"
    blank = next(
    ...
    def model_focus(slide, content, slide_number):
        ...
        if native_asset.poster_part:
            try:
                ...
                poster.name = f"!!3D Poster — {native_asset.description or native_asset.name}"
            except Exception:
                pass
    ...
    *,
    ...
    try:
        ...
        if _presentation_quota_cooldown_active() and mode in {"create", "redesign"}:
            ...
            warnings.append(
                "Gemini presentation planning is in a short quota cooldown. "
                "A minimal local deck was completed without another API request."
            )
            qa["quota_fallback"] = {
                "used": True,
            ...
            try:
                ...
                ai_quota_exhausted = True
                warnings.append(
                    "Gemini planning quota was unavailable. A minimal local deck "
                    "was completed instead of failing or making more API requests."
                )
                qa["quota_fallback"] = {
                    "used": True,
                    "stage": "planning",
                            ...
                            try:
                                ...
                                if _is_quota_error(exc):
                                    ...
                                    warnings.append(
                                        ...
                                        "Kept the verified PowerPoint instead of failing."
                                    )
                                else:
...
def create_presentation(parameters: dict, response=None, player=None, session_memory=None) -> str:
    ...
    rest of the assistant's action interface.  Presentation creation is intentionally a
