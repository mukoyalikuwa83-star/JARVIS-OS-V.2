...
"""
    ...
    def _login_message(self) -> str:
        return "Instagram login required. I opened the Instagram browser; log in there, then try again."
...
"""
    ...
    def _resolve_chat(self, page, receiver: str, message: str) -> tuple[dict | None, str | None]:
        ...
        if not all_rows:
            ...
            if self._limit_modal_visible(page):
                return None, (
                    ...
                    "Close that popup in the Instagram browser, then try again. "
                    f"Debug: url={url or 'unknown'}, title={title or 'unknown'}, visible_text={body_sample or 'none'}"
                )
            return None, (
                "I could not read your Instagram DM list. Log into the Instagram browser and open Direct Messages, then try again. "
                f"Debug: url={url or 'unknown'}, title={title or 'unknown'}, visible_text={body_sample or 'none'}"
            )
        ...
        return None, (
            ...
            "Open that DM in the Instagram browser and leave the recipient blank, or message someone from the visible/recent DM list."
        )
...
"""
    ...
    def _prepare_draft(self, receiver: str, message: str) -> str:
        ...
        if recipient:
            ...
            if not recipient:
                return "Open an Instagram DM in the Instagram browser, or provide a recipient name."

        if not self._fill_composer(page, message):
            return "I could not type into the Instagram composer. Keep the Instagram browser on the desired DM and try again."
    ...
    def _send_draft(self) -> str:
        ...
        if not self._pending:
            return "Could not find a pending Instagram draft. Ask the assistant to prepare the Instagram message again."
        self._dismiss_interruptions(page)
