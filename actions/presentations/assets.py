...
IMAGE_QUOTA_COOLDOWN_SECONDS = 30
...
def _image_request_limit(request: PresentationRequest, slide_total: int) -> int:
    """Generate imagery for every slide that needs it; capped only by deck size."""
...
def generate_planned_assets(
    ...
    if _image_quota_cooldown_active():
        ...
        warnings.append(
            "Gemini image generation is in a short quota cooldown. The assistant skipped new image "
        ...
        for completed, (slide_number, prompt) in enumerate(selected_prompts, start=1):
            ...
            try:
                ...
                if _is_quota_error(exc):
                    ...
                    warnings.append(
                        "Gemini image quota was reached. The assistant stopped additional image requests "
