...
ERROR_ANALYST_PROMPT = """You are the error recovery module of a personal AI assistant.
...
- Max retries: how many times to retry if decision is retry (1 to 4)
...
def _generate_reply(prompt: str, system_instruction: str | None = None) -> str:
    """Call Gemini through whichever SDK is installed (google-genai preferred)."""
    api_key = _get_api_key()
    if not api_key:
        raise RuntimeError("No Gemini API key configured (env GEMINI_API_KEY or config/api_keys.json).")
    ...
    model = legacy_genai.GenerativeModel(model_name="gemini-3.5-flash-lite", **kwargs)
...
Attempt number: {attempt}"""
    ...
    try:
        text = _generate_reply(prompt, system_instruction=ERROR_ANALYST_PROMPT)
