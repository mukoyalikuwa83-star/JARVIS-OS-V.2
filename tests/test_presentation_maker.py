...
class PresentationMakerTests(unittest.TestCase):
    ...
    def test_model_focus_balances_native_3d_against_only_essential_text(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            model_slides = []
            for slide in deck.slides:
                models = [shape for shape in slide.shapes if shape.name.startswith("!!3D Model")]
                if models:
                    model_slides.append(slide)
                    posters = [shape for shape in slide.shapes if shape.name.startswith("!!3D Poster")]
                    text_shapes = [
    ...
    def test_default_quality_generates_imagery_for_every_qualified_slide(self):
        ...
        fast = PresentationRequest.from_parameters({"topic": "Biology", "quality": "fast"})

        self.assertEqual(presentation_assets._image_request_limit(standard, 12), 12)
        self.assertEqual(presentation_assets._image_request_limit(premium, 12), 12)
        self.assertEqual(presentation_assets._image_request_limit(fast, 12), 0)
        self.assertEqual(presentation_assets._image_request_limit(standard, 0), 0)
