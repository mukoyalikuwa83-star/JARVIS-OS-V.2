...
class QAModeSafetyTests(unittest.TestCase):
    ...
    def test_live_reconnect_backoff_is_bounded(self):
        ...
        delays = [main._live_reconnect_delay(attempt) for attempt in range(1, 9)]
        for delay in delays:
            self.assertGreaterEqual(delay, 0.0, f"delay {delay} must be non-negative")
            self.assertLessEqual(delay, main._LIVE_RECONNECT_MAX_DELAY, f"delay {delay} exceeds max")
    ...
    def test_independent_tool_batch_executes_concurrently_and_preserves_order(self):
        ...
        # Use one persistent loop so event-loop creation cost is not part of the
        # timing, and compare the parallel batch against its own sequential
        # baseline so the assertion is immune to slow or loaded machines.
        ...
        async def run_sequential():
            output = []
            for call in calls:
                output.append(await execute(call))
            return output
        ...
        started = time.perf_counter()
        results = loop.run_until_complete(jarvis._execute_tool_batch(calls))
        elapsed = time.perf_counter() - started
        loop.close()
        self.assertEqual(results, ["weather_report", "web_search"])
        self.assertLessEqual(elapsed, sequential_elapsed * 2 + 1.0)
